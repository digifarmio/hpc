# Running 29 Million Fields Through Betzy: Vector Tiles, GPU Training, and Lustre's Limits

*How we used Norway's Betzy cluster for A100 GPU training, continental-scale tile generation, and crop insurance batch processing -- and what Betzy's unique constraints taught us about HPC job design.*

---

Betzy, NRIS/Sigma2's compute-focused cluster in Norway, played three distinct roles in our geospatial ML work: DDP training on A100 GPUs for crop segmentation, processing 29.1 million US crop field polygons into web-ready vector tiles, and batch-running 13,510 crop insurance assessments. Each workload exposed different aspects of Betzy's personality.
...
Our solution for the fields pipeline: process 4 years simultaneously, one per node, using `srun --exclusive --nodes=1` inside a 4-node allocation. A simple shell loop fills all 4 nodes with `srun &` background processes and uses `wait -n` to release a node when any year finishes:

```bash
for year in "${YEARS[@]}"; do
  while (( $(jobs -r | wc -l) >= 4 )); do wait -n; done
  srun --exclusive --nodes=1 bash process_year.sh "$year" &
done
wait
```

15 years of data, 4 at a time, completed in ~8-10 hours. Turning a constraint into parallelism.

## The Tippecanoe Bug Nobody Documented

Our most dramatic bug was in tippecanoe, the vector tile generator. After streaming all 29.1 million features (~30 minutes of work), tippecanoe crashed with: `Internal error: 745 shards not a power of 2`.

The root cause is a subtle interaction between Betzy's hardware and tippecanoe's internal shard calculation. Betzy exposes 256 logical CPUs (128 physical cores with hyperthreading). Tippecanoe computes `TEMP_FILES` from the open file limit (~1500), which yields 745 -- not a power of 2. On most machines, `CPUS * 4` caps this to a power of 2. But on Betzy, `256 * 4 = 1024 > 745`, so the cap doesn't apply, and the power-of-2 assertion fails.

The fix: `export TIPPECANOE_MAX_THREADS=128`. One environment variable, but only discoverable by reading tippecanoe's C++ source. The `-j` flag, which looks like it controls threads, is actually a GeoJSON feature filter.

## Lustre vs. RAM: A 100x Performance Difference

After fixing the shards bug, tippecanoe still didn't work. It would stream features, then sit at 1 thread with 0% CPU for hours. The MBTiles file stayed at 28KB (header only).

The diagnosis: tippecanoe creates 500-700 temporary files for internal sorting. With `--temporary-directory` pointed at Lustre (`/cluster/work/`), each file operation goes through the parallel filesystem. Lustre excels at large sequential I/O but has terrible latency for small random operations. With 4 tippecanoe processes across 4 nodes all hitting the same Lustre filesystem, I/O contention was catastrophic.

The fix: use `/dev/shm` (RAM-backed tmpfs) for temp files. Betzy provides a generous 126GB of `/dev/shm` per node -- more than enough for tippecanoe's ~15GB of temporary data. Performance went from "never completes" to 45 minutes.
...
2. **`os.cpu_count()` returns 256, not 128.** Hyperthreading means tools see double the physical core count. This breaks tippecanoe and likely other tools that use CPU count for internal sizing. Cap thread counts at 128.
